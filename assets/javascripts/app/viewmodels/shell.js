define(['plugins/router', 'durandal/app', 'typeahead', 'jquery-text-selection', 'activities','logsearch'], 
	function (router, app, typeahead, jts, activities, logsearch) {
		...
		var add = function(filter) {
			...
			if (router.activeInstruction().fragment != "logs") {
				console.log("#logs");
				router.navigate('#logs');
				setTimeout(function() {
					logsearch.filters.removeAll();
		...
		var trytoadd = function() {
			...
			if (s && s!="") {
				...
				if (type === 'key-value') {
					...
					$('#search').typeahead('setQuery', '');
					console.log("repo? type="+type+", s="+s);
					if (router.activeInstruction().fragment != "repositories") {
		...
		return {
				...
				activate: function () {
						router.map([
								...
								{ route: 'repositories', title:'Repository Results', moduleId: 'viewmodels/reporesults', nav: true }
						]).buildNavigationModel();
						activities.setWorking(true, "routing");
